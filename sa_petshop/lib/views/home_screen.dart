  ...
  @override
    Widget build(BuildContext context) {
      return Scaffold(
        ...
        body: _isLoading
        ? Center(child: CircularProgressIndicator(),) // enquanto estiver cerregado as info do BD , vai mostrar uma barra circular
        : Padding(
            padding: EdgeInsets.all(16), // espacamento da parede do aplicativo de 16 px
            child: ListView.builder( // construtor da lista
              itemCount: _pets.length, // tamanho da lista
              itemBuilder: (context,index){ //metodo da contrucao da lista 
                final pet = _pets[index];
                return ListTile(
                  title: Text("${pet.nome} - ${pet.raca}"),
                  subtitle: Text("${pet.nomeDono} - ${pet.telefone}"),
                  // on tap -> import 'package:sa_petshop/views/cadastro_pet_screen.dart';
